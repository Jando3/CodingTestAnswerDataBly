...
namespace CodingTest.TestQuestions.Question1
{
    public class BatchRepository
    {
        ...
        public IEnumerable<BatchesViewModel> GetUncommittedBatches()
        {
            ...
            //BatchView return of uncommited classes.
            return _batchEntities
           .Where(batchEntities => batchEntities.Status == BatchStatus.Uncommitted)
           .Select(batchEntities => new BatchesViewModel
           {
               BatchId = batchEntities.BatchId,
               Status = batchEntities.Status.ToString(),
               CreatedBy = batchEntities.CreatedByUser.Name,
               CreatedDateTime = batchEntities.CreatedDateTime
           });
        }
    }
